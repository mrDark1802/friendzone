export function renderVerificationEmailHtml(payload: {
  displayName: string;
  verificationUrl: string;
  expiresInMinutes: number;
}): string {
  const { displayName, verificationUrl, expiresInMinutes } = payload;
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Verify your FriendZone account</title>
  <style>
    ...
    .link-fallback { word-break: break-all; font-size: 12px; color: #818cf8; margin-top: 16px; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <p>Welcome to FriendZone! Please confirm your email address to complete registration and unlock full communication features across language barriers.</p>
    
    <div class="button-wrapper">
      <a href="${verificationUrl}" target="_blank" class="button">Verify My Email</a>
    </div>

    <p>This verification link will expire in <strong>${expiresInMinutes} minutes</strong>.</p>
    <p>If you didn't create a FriendZone account, you can safely ignore this email.</p>

    <p class="link-fallback">Button not working? Copy and paste this URL into your browser:<br><a href="${verificationUrl}" style="color: #818cf8;">${verificationUrl}</a></p>

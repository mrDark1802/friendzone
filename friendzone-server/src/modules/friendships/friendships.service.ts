...
export class FriendshipsService {
  ...
  async getFriendsList(userId: string) {
    const friendships = await prisma.friendship.findMany({
      ...
      include: {
        user1: { select: { id: true, username: true, email: true, displayName: true, nativeLanguage: true, profileMediaId: true } },
        user2: { select: { id: true, username: true, email: true, displayName: true, nativeLanguage: true, profileMediaId: true } },
      },
    });
